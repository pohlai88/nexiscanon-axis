...
function App() {
  ...
  return <CommandKPalette isOpen={isOpen} onClose={close} actions={actions} />;
}
```
...
<CommandKPalette isOpen={isOpen} onClose={close} groups={commandGroups} />;
```
...
function App() {
  ...
  return (
    <>
      ...
      <CommandKPalette isOpen={isOpen} onClose={close} actions={actions} />
    </>
  );
}
```
...
const getDynamicActions = () => {
  const userActions = currentUser.role === 'admin' ? adminActions : userActions;

  return [...commonActions, ...userActions];
};

<CommandKPalette actions={getDynamicActions()} />;
```
