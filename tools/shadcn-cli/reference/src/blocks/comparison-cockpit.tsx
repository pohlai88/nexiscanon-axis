...
export function ComparisonCockpit({
  ...
  const handleTextSelection = () => {
    ...
    if (text && text.length > 0) {
      ...
      if (rect) {
        ...
        if (aiAssisted) {
          const suggestedField = findBestFieldMatch(
            text,
            rightDocument.fields || [],
            fieldMappings,
          );
          if (suggestedField) {
  ...
  const handleScroll = (source: 'left' | 'right') => {
    ...
    if (sourceScroll && targetScroll) {
      const scrollPercentage =
        ...
        (sourceScroll.scrollHeight - sourceScroll.clientHeight);
      targetScroll.scrollTop =
        scrollPercentage *
        (targetScroll.scrollHeight - targetScroll.clientHeight);
    }
  };
  ...
  const leftWidth =
    maximizedPanel === 'left' ? 100 : maximizedPanel === 'right' ? 0 : split;
  const rightWidth =
  ...
  const validationStatus = React.useMemo(() => {
    ...
    const requiredFilled = rightDocument.fields.filter(
  ...
  return (
    ...
    >
        ...
        <div className="bg-muted/30 flex items-center justify-between border-b px-4 py-3">
          ...
          <div className="flex items-center gap-1">
            <Button
              ...
              size="icon"
              onClick={() =>
                setMaximizedPanel(maximizedPanel === 'left' ? null : 'left')
              }
              className="h-7 w-7"
            >
              {maximizedPanel === 'left' ? (
      ...
      {!maximizedPanel && (
        <div
          className={cn(
            'group bg-border hover:bg-primary relative flex w-1 cursor-col-resize items-center justify-center transition-colors',
        ...
        >
          ...
          <div
            className={cn(
              'bg-border group-hover:bg-primary absolute h-12 w-1 rounded-full transition-colors',
              isDragging && 'bg-primary',
            )}
          />
        </div>
        ...
        <div className="bg-muted/30 flex items-center justify-between border-b px-4 py-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              size="icon"
              onClick={() =>
                setMaximizedPanel(maximizedPanel === 'right' ? null : 'right')
              }
              className="h-7 w-7"
            >
              {maximizedPanel === 'right' ? (
          ...
          <div className="space-y-4 p-6">
            {rightDocument.fields?.map((field) => (
              <FormField
                ...
                field={field}
                onUpdate={(value) =>
                  onFieldUpdate?.(rightDocument.id, field.id, value)
                }
                aiSuggestion={
                  (aiAssisted &&
                  highlights.find((h) => h.targetFieldId === field.id)?.text) || undefined
                }
              />
            ))}
          </div>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4 text-sm">
                <span className="text-muted-foreground">
                  Progress:{' '}
                  <strong>
                    {validationStatus.filled}/{validationStatus.total}
                  </strong>
                </span>
                {validationStatus.required > 0 && (
                  <span
                    className={cn(
                      validationStatus.requiredFilled ===
      ...
      {selectedText && (
        ...
        >
          <div className="border-primary bg-primary/10 animate-pulse rounded-lg border-2 px-2 py-1 text-xs font-medium shadow-lg">
            {selectedText.text}
          </div>
        </div>
      )}
    ...
    >
      <div className="flex items-center gap-2">
        ...
        {isHovered && targetField && (
          <div className="bg-popover rounded-md border px-2 py-1 text-xs shadow-lg">
            <div className="flex items-center gap-1">
