...
export function ExcelModeGrid<T extends GridRow = GridRow>({
  ...
  onSave,
  enableDragFill = true,
  enableBulkEdit: _enableBulkEdit = true,
  ...
  const [selectedCells, _setSelectedCells] = React.useState<Set<string>>(
    new Set(),
  );
  ...
  const [dragFillStart, setDragFillStart] = React.useState<{
    rowId: string;
    columnId: string;
  } | null>(null);
  ...
  const _cellRefs = React.useRef<Map<string, HTMLDivElement>>(new Map());
  ...
  const handleDragFillEnd = (endRowId: string) => {
    ...
    const sourceRow = data[startRowIndex];
    if (!sourceRow) return;

    const sourceValue = sourceRow[dragFillStart.columnId];
  ...
  const handleCopy = () => {
    if (selectedCells.size === 0 && focusedCell) {
      ...
      if (row) {
        setCopiedCells([
          { value: row[focusedCell.columnId], columnId: focusedCell.columnId },
        ]);
      }
    } else {
      const copied: any[] = [];
  ...
  // Get cell key
  const getCellKey = (rowId: string, columnId: string) =>
    `${rowId}::${columnId}`;
                ...
                >
                  {columns.map((column) => (
                    <GridCell
                      ...
                      enableDragFill={
                        (enableDragFill === true && column.editable && !readOnly) ?? false
                      }
...
function GridCell<T extends GridRow>({
  ...
  return (
    <td
      className={cn(
        'group border-border relative border px-3 py-2 text-sm transition-colors',
      ...
      {isEditing ? (
        ...
        <>
          ...
          {enableDragFill && isFocused && (
            <div
              className="border-primary bg-primary absolute -right-1 -bottom-1 h-3 w-3 cursor-crosshair rounded-sm border-2 opacity-0 transition-opacity group-hover:opacity-100"
              onMouseDown={(e) => {
                e.preventDefault();
                onDragFillStart();
              }}
