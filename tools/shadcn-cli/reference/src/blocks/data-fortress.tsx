...
export function DataFortress<T = any>({
  ...
  resizableColumns: _resizableColumns = true,
  reorderableColumns: _reorderableColumns = false,
}: DataFortressProps<T>) {
  // State
  const [columns, setColumns] = React.useState(initialColumns);
  const [columnWidths, _setColumnWidths] = React.useState<
    Record<string, number>
  >({});
  const [sort, setSort] = React.useState<DataFortressSortConfig[]>(defaultSort);
  const [_filters, _setFilters] =
...
type DataFortressToolbarProps<_T = unknown> = {
  searchable: boolean;
  searchPlaceholder: string;
  searchQuery: string;
  onSearch: (query: string) => void;
...
function DataFortressToolbar<_T>({
  searchable,
  searchPlaceholder,
  searchQuery,
  onSearch,
