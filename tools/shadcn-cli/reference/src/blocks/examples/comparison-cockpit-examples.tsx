...
import { ComparisonCockpit, type ComparisonDocument, type DocumentField } from '../comparison-cockpit';
import type { AuditEntry } from '../audit-trail-viewer';
  ...
  const handleFieldTransfer = (sourceText: string, targetFieldId: string) => {
    ...
    const auditEntry: AuditEntry = {
      ...
      resource: {
        ...
        id: targetFieldId,
        name:
          formFields.find((f) => f.id === targetFieldId)?.label ||
          targetFieldId,
      },
      changes: [
        {
      ...
      <div className="flex-1 overflow-hidden pt-16">
        <ComparisonCockpit
          ...
          onFieldUpdate={(docId, fieldId, value) => {
            setFormFields((prev) =>
              prev.map((f) =>
                f.id === fieldId ? { ...f, value, validated: true } : f,
              ),
            );
          }}
  ...
  return (
    <div className="h-screen">
      <ComparisonCockpit
        ...
        onFieldTransfer={(text, fieldId) =>
          console.log(`Transfer: ${text} → ${fieldId}`)
        }
      />
    </div>
  );
