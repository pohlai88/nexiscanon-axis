...
import { DoubleScreenLayout } from '../double-screen-layout';
import { NavbarOverlay } from '../navbar-overlay';
import { AuditTrailViewer, type AuditEntry } from '../audit-trail-viewer';
import { CommandKPalette, useCommandK, CommonCommands, type CommandAction } from '../command-k-palette';
...
export function CompleteSystemWithCommandK() {
  ...
  const exportAuditLog = () => {
    const csv = auditEntries
      .map(
        (e) => `${e.timestamp},${e.user.name},${e.action},${e.resource.name}`,
      )
      .join('\n');
    ...
    <div className="space-y-6 p-6">
      <div>
        ...
        <p className="text-muted-foreground">
          Press <kbd className="bg-muted rounded px-2 py-1 text-xs">⌘K</kbd> to
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        <div className="bg-card rounded-lg border p-6">
          <h3 className="mb-2 font-semibold">Q4 Financial Report</h3>
          <p className="text-muted-foreground text-sm">
            Last modified 2 hours ago
          </p>
        </div>
        <div className="bg-card rounded-lg border p-6">
          <h3 className="mb-2 font-semibold">Marketing Strategy</h3>
          <p className="text-muted-foreground text-sm">
            Last modified 5 hours ago
          </p>
        </div>
        <div className="bg-card rounded-lg border p-6">
          <h3 className="mb-2 font-semibold">Product Roadmap</h3>
          <p className="text-muted-foreground text-sm">
...
export function MinimalCommandKExample() {
  ...
  return (
    <div>
      ...
      {/* Command Palette */}
      <CommandKPalette isOpen={isOpen} onClose={close} actions={quickActions} />
    </div>
  );
}
...
export function CommandKWithRecentActions() {
  const { isOpen, open: _open, close } = useCommandK();
