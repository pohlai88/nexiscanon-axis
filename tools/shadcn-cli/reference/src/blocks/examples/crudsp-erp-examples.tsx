...
import { CRUDSPToolbar, ERPModulePresets, type CRUDSPAction } from '../crudsp-toolbar';
import { InteractiveDataTable } from '../interactive-data-table';
import { NavbarOverlay } from '../navbar-overlay';
import { CommandKPalette, useCommandK } from '../command-k-palette';
import { AuditTrailViewer, type AuditEntry } from '../audit-trail-viewer';
...
export function SalesOrderModule() {
  ...
  const [loading, setLoading] = React.useState<
    Partial<Record<CRUDSPAction, boolean>>
  >({});
  const [auditEntries, setAuditEntries] = React.useState<AuditEntry[]>([]);

  // Sales order data
  const [orders, _setOrders] = React.useState([
    {
      id: 'SO-001',
      customer: 'Acme Corp',
      amount: '$12,500',
  ...
  const handlers = {
    ...
    delete: () => {
      ...
      const order = selectedOrders[0];
      if (!order) return;
      if (confirm(`Cancel order ${order}?`)) {
        console.log('Cancelling order:', order);
        addAuditEntry('delete', order);
      }
    },
    search: () => {
      ...
      <div className="flex-1 overflow-hidden pt-16">
        <div className="h-full space-y-6 p-6">
          ...
          {selectedOrders.length > 0 && (
            <div className="bg-primary/10 rounded-lg border p-3 text-sm">
          ...
          {/* Audit Trail Panel */}
          {showAuditTrail && (
            <div className="bg-background fixed top-16 right-0 bottom-0 w-96 overflow-auto border-l shadow-lg">
              <AuditTrailViewer
  ...
  return (
    <div className="p-6">
      ...
      <div className="text-muted-foreground rounded-lg border p-12 text-center">
        ...
        <p className="mt-2 text-sm">Click Predict to see AI reorder alerts</p>
      </div>
    </div>
  );
}
...
export function CRUDSPWithPermissions() {
  const [userRole, setUserRole] = React.useState<'admin' | 'editor' | 'viewer'>(
    'editor',
  );
