...
```tsx
import { MagicApprovalTable, MagicApprovalItem } from "@workspace/shared-ui/blocks";
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">
          {approvals.filter((a) => a.status === "pending").length} items awaiting your decision
        </p>
      </div>
...
const todos = await fetch(
  `/api/todos?tenantId=${tenantId}&caseId=${caseId}`
);
```
...
const approveAll = async (items: MagicApprovalItem[]) => {
  await Promise.all(
    items.map((item) => handleApprove(item, "Bulk approval"))
  );
};
```
