...
export interface AuditEntry {
  ...
  action: "create" | "update" | "delete" | "view" | "download" | "login" | "logout" | "error";
  resource: {
    type: string;
    id: string;
    name: string;
...
function AuditEntryItem({
  ...
  return (
    <div
      className={cn(
        "group relative flex gap-4 cursor-pointer",
        compactMode && "gap-3"
      )}
      onClick={onClick}
    >
      ...
      <div className="relative z-10 flex-shrink-0">
        <div
          className={cn(
            "flex h-11 w-11 items-center justify-center rounded-full border-4 border-background",
      ...
      {/* Content */}
      <div className="flex-1 rounded-lg border bg-card p-4 transition-all group-hover:shadow-md">
        <div className="flex items-start justify-between gap-3 mb-2">
          <div className="flex-1">
            ...
            <p className="text-sm text-foreground">
              ...
              <span className="text-muted-foreground">{entry.resource.type}</span>{" "}
          ...
          <div className="flex items-center gap-2">
            {entry.severity && (
              <Badge variant="secondary" className={cn("text-xs", severityColors[entry.severity])}>
                {entry.severity}
              </Badge>
            )}
            {StatusIcon && (
