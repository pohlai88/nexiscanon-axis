...
import { ResizableHandle, ResizablePanel, ResizablePanelGroup } from "@workspace/design-system/components/resizable";
import { Drawer, DrawerClose, DrawerContent, DrawerDescription, DrawerFooter, DrawerHeader, DrawerTitle } from "@workspace/design-system/components/drawer";
...
export type MagicApprovalTableProps = {
  ...
  // Callbacks
  onApprove?: (item: MagicApprovalItem, comment?: string) => void | Promise<void>;
...
export function MagicApprovalTable({
  ...
  const [selectedItem, setSelectedItem] = React.useState<MagicApprovalItem | null>(null);
                      ...
                      {columns.map((col) => (
                        <TableCell key={col.id}>
                          <CellRenderer
                            ...
                            onEdit={(value) => handleCellEdit(item.id, col.id, value)}
                            onStartEdit={() =>
                              allowInlineEdit &&
                              col.editable &&
...
function CellRenderer({ item, column, isEditing, onEdit, onStartEdit }: CellRendererProps) {
  ...
  return (
    <div
      className={`${column.editable ? "cursor-pointer hover:bg-muted/50 px-2 py-1 rounded" : ""}`}
...
function StatusBadge({ status }: { status: ApprovalStatus }) {
  const config = {
    ...
    review: { icon: AlertCircle, variant: "secondary" as const, label: "Review" },
  };
...
function ApprovalDetailsDrawer({
  ...
  const [linkedTodoId, setLinkedTodoId] = React.useState(item.linkedTodoId || "");
      ...
      <DrawerContent className="max-w-2xl">
        ...
        <div className="p-6 space-y-6 max-h-[60vh] overflow-y-auto">
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              ...
              {item.dueDate && (
                <div>
                  ...
                  <div className="mt-1">{new Date(item.dueDate).toLocaleDateString()}</div>
                </div>
              )}
            </div>
            {item.description && (
          ...
          <div className="space-y-2">
            <h3 className="font-semibold text-sm">Link to Task</h3>
            <div className="flex items-center gap-2">
              <Input
