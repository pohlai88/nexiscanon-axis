...
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@workspace/design-system/components/card";
    ...
    >
      ...
      <div
        className={cn(
          ...
          vertical
            ? "inset-x-0 top-0 h-1/6 bg-gradient-to-b from-background to-transparent"
            : "inset-y-0 left-0 w-1/12 bg-gradient-to-r from-background to-transparent"
        )}
      />
      <div
        className={cn(
          "pointer-events-none absolute z-10",
          vertical
            ? "inset-x-0 bottom-0 h-1/6 bg-gradient-to-t from-background to-transparent"
            : "inset-y-0 right-0 w-1/12 bg-gradient-to-l from-background to-transparent"
        )}
      />
...
export function LogoMarquee({
  ...
  const items: MarqueeItem[] = logos.map((logo) => ({
    id: logo.id,
    content: (
      <div className="flex h-16 items-center justify-center rounded-lg border bg-card p-4 transition-colors hover:bg-muted">
        <img
          src={logo.src}
          alt={logo.alt}
...
export function TestimonialMarquee({
  ...
  return <Marquee items={items} speed="normal" direction="right" className={className} />;
}
