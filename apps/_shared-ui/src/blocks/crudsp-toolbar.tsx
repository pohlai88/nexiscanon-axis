...
export type CRUDSPAction = "create" | "read" | "update" | "delete" | "search" | "audit" | "predict";
...
export function CRUDSPToolbar({
  ...
  const defaultConfigs: Record<CRUDSPAction, Omit<CRUDSPButtonConfig, "onClick">> = {
    create: {
      action: "create",
      label: "Create",
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        ...
        >
          ...
          {config.badge && (
            <Badge
              variant={config.variant === "destructive" ? "destructive" : "secondary"}
...
export const ERPModulePresets = {
   ...
   */
  salesOrders: (handlers: Record<string, () => void>): Partial<Record<CRUDSPAction, CRUDSPButtonConfig>> => ({
    create: {
      action: "create",
      label: "New Order",
      icon: <Plus className="h-4 w-4" />,
   ...
   */
  inventory: (handlers: Record<string, () => void>): Partial<Record<CRUDSPAction, CRUDSPButtonConfig>> => ({
    create: {
      action: "create",
      label: "Add Item",
      icon: <Plus className="h-4 w-4" />,
   ...
   */
  crm: (handlers: Record<string, () => void>): Partial<Record<CRUDSPAction, CRUDSPButtonConfig>> => ({
    create: {
      action: "create",
      label: "New Contact",
      icon: <Plus className="h-4 w-4" />,
   ...
   * Human Resources (HR)
   */
  hr: (handlers: Record<string, () => void>): Partial<Record<CRUDSPAction, CRUDSPButtonConfig>> => ({
    create: {
      action: "create",
      label: "New Employee",
      icon: <Plus className="h-4 w-4" />,
   ...
   */
  finance: (handlers: Record<string, () => void>): Partial<Record<CRUDSPAction, CRUDSPButtonConfig>> => ({
    create: {
      action: "create",
