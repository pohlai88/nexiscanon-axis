...
export interface DocumentTemplate {
  id: string;
  name: string;
  type: "drivers_license" | "insurance_card" | "passport" | "id_card" | "custom";
...
export function MagicPaste({
  ...
  const [detectedTemplate, setDetectedTemplate] = React.useState<DocumentTemplate | null>(null);
  const [snipMode, setSnipMode] = React.useState(false);
  const [snipBox, setSnipBox] = React.useState<{ x: number; y: number; width: number; height: number } | null>(null);
  ...
  const processOCR = async (imageData: string, box?: { x: number; y: number; width: number; height: number }) => {
        ...
        <CardContent className="space-y-4">
          ...
          {!image && !useWebcam && (
            <div className="grid gap-4 md:grid-cols-2">
              <Card className="cursor-pointer transition-all hover:shadow-lg" onClick={() => fileInputRef.current?.click()}>
              ...
              {enableWebcam && (
                <Card className="cursor-pointer transition-all hover:shadow-lg" onClick={startWebcam}>
                  <CardContent className="flex flex-col items-center justify-center p-8 text-center">
          ...
          {image && !useWebcam && (
            <div className="space-y-4">
              ...
              {detectedTemplate && (
                <div className="flex items-center justify-between rounded-lg border bg-primary/10 p-4">
                  <div className="flex items-center gap-3">
                    {detectedTemplate.icon}
                    <div>
                      <p className="font-semibold">Detected: {detectedTemplate.name}</p>
                      <p className="text-sm text-muted-foreground">
                        {detectedTemplate.fields.length} fields identified
                      </p>
                    </div>
                  </div>
                  <Button onClick={() => processOCR(image)} disabled={isProcessing}>
                    {isProcessing ? (
                      <>
              ...
              <div className="relative">
                ...
                >
                  ...
                  {detectedTemplate?.fields.map((field, idx) => (
                    field.location && (
                      <div
                        key={idx}
                        ...
                        {ocrResults.map((field) => (
                          ...
                          >
                            <div className="flex-1">
                              ...
                              <p className="mt-1 font-mono text-sm">{field.value}</p>
...
const DEFAULT_TEMPLATES: DocumentTemplate[] = [
  {
    ...
    fields: [
      { id: "name", label: "Full Name", location: { x: 10, y: 15, width: 40, height: 8 } },
      { id: "dob", label: "Date of Birth", location: { x: 10, y: 35, width: 30, height: 6 } },
      { id: "license_number", label: "License #", location: { x: 10, y: 50, width: 35, height: 6 } },
      { id: "address", label: "Address", location: { x: 10, y: 65, width: 50, height: 10 } },
      { id: "expiry_date", label: "Expiry Date", location: { x: 10, y: 80, width: 25, height: 6 } },
    ],
  },
  {
    id: "insurance_card",
    ...
    fields: [
      { id: "member_id", label: "Member ID", location: { x: 10, y: 20, width: 40, height: 8 } },
      { id: "policy_number", label: "Policy #", location: { x: 10, y: 35, width: 40, height: 8 } },
      { id: "group_number", label: "Group #", location: { x: 10, y: 50, width: 30, height: 8 } },
      { id: "name", label: "Name", location: { x: 10, y: 65, width: 40, height: 8 } },
    ],
  },
  {
    id: "passport",
    ...
    fields: [
      { id: "passport_number", label: "Passport #", location: { x: 10, y: 15, width: 35, height: 8 } },
      { id: "name", label: "Name", location: { x: 10, y: 30, width: 40, height: 8 } },
      { id: "dob", label: "Date of Birth", location: { x: 10, y: 45, width: 30, height: 6 } },
      { id: "issue_date", label: "Issue Date", location: { x: 10, y: 60, width: 25, height: 6 } },
      { id: "expiry_date", label: "Expiry Date", location: { x: 45, y: 60, width: 25, height: 6 } },
    ],
  },
  {
    id: "custom",
