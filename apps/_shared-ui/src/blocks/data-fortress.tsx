...
export function DataFortress<T = any>({
  ...
  const [filters, setFilters] = React.useState<DataFortressFilterConfig[]>(defaultFilters);
  ...
  const [auditDrawerRow, setAuditDrawerRow] = React.useState<DataFortressRow<T> | null>(null);
              ...
              {data.map((row, index) => (
                <TableRow
                  ...
                  className={`${striped && index % 2 === 0 ? "bg-muted/50" : ""
                    } ${rowClassName?.(row) || ""} ${selected.has(row._id) ? "bg-primary/5" : ""
                    } ${compact ? "h-10" : ""}`}
                >
                  {selectable && (
                    <TableCell>
                  ...
                  {visibleColumns.map((col) => {
                    ...
                    const formatted = col.format && value ? col.format(value) : value;
            ...
            <div className="p-4">
              {auditDrawerRow?._audit ? (
                ...
                <div className="flex flex-col items-center justify-center py-12 space-y-2">
                  ...
                  <p className="text-sm text-muted-foreground">This record has no change history</p>
                </div>
              )}
            </div>
            <DrawerFooter>
