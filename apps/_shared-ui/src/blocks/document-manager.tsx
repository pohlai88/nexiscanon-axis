  ...
  return (
    ...
    >
      <CardContent className="p-4">
        {/* Thumbnail/Icon */}
        <div className="mb-3 flex items-center justify-center rounded-lg bg-muted p-6">
          {doc.thumbnail ? (
            <img src={doc.thumbnail} alt={doc.name} className="h-24 w-full object-cover rounded" />
          ) : (
            getFileIcon()
          )}
        </div>
        ...
        <div className="space-y-2">
          <div className="flex items-start justify-between gap-2">
            <h4 className="line-clamp-2 text-sm font-medium leading-tight">{doc.name}</h4>
            <button
              onClick={onToggleStar}
              className="flex-shrink-0 text-muted-foreground hover:text-yellow-500"
            >
              {doc.starred ? (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          ...
          {doc.starred && <Star className="h-3 w-3 fill-yellow-500 text-yellow-500" />}
        </div>
        <div className="flex items-center gap-2 text-xs text-muted-foreground">
          <span>{formatFileSize(doc.size)}</span>
          <span>•</span>
