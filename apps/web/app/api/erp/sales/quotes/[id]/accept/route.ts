// apps/web/app/api/erp/sales/quotes/[id]/accept/route.ts
// Sales Quote accept transition endpoint
...
/**
 * POST /api/erp/sales/quotes/:id/accept
 * Accept a quote (SENT → ACCEPTED)
 */
export const POST = kernel({
  method: "POST",
  routeId: "erp.sales.quotes.accept",
  ...
  async handler({ params, tenantId, actorId, ctx }) {
    ...
    const quote = await quoteService.accept(
      {
        tenantId,
        actorUserId: actorId,
        traceId: ctx.traceId,
