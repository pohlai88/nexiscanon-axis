...
if (spec.idempotent) {
  ...
  if (idempotencyKey) {
    const cached = await checkIdempotency(tenantId, idempotencyKey, spec.routeId, spec.method, requestHash);
    if (cached.exists) {
...
export async function createWorkerUtils(connectionString: string): Promise<WorkerUtils>;
...
await runWorker(process.env.DATABASE_URL!, {
...
export async function createKey(input: CreateKeyInput): Promise<{ keyAlias: string; version: number }>;
export async function sign(input: SignRequest): Promise<SignResult>;
export async function rotateKey(tenantId: string, keyAlias: string, reason: string): Promise<{ newVersion: number }>;
```
...
export type RouteSpec<TQuery, TBody, TOutput> = {
  ...
  body?: TBody;
  output: TOutput;
  handler: (ctx: HandlerContext<z.infer<TQuery>, z.infer<TBody>>) => Promise<z.infer<TOutput>>;
};
```
...
function validateSchema<T extends z.ZodTypeAny>(
  ...
  if (!result.success) {
    ...
    throw new KernelError(ErrorCode.VALIDATION_ERROR, `${schemaName} validation failed`, undefined, fieldErrors);
  }
